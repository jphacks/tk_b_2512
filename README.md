...
#### 2. 特長2(独自性)
今までのecサイトは購入履歴を元にレコメンド
今回はユーザの現状の願望、意思にアプローチ
これまでのECサイトは、お客様の購入履歴にもとづいて商品をおすすめしていました。 私たちは、お客様の「今のテーブルに合わせたい」という想いを大切にします。AIがあなたのセンスを汲み取り、新しいコーディネートの可能性を広げます
...
### 解決出来ること
購入前の不安を取り除くことができる。
イメージを可視化することで、実際の配置を購入前に疑似体験することができる。
オンラインで器を購入する時、手持ちの食器との相性を事前に確認できず、困ることがある。

### 今後の展望
商品リストを実在するecサイトから取得する
プロトタイプでは、事前に入力済みのデータを用いて表示
メモ：2段階のそれぞれの役割を明確にし、プロンプトを調整する必要があるかも？今はブラックボックス
既存のテーブルコーディネートも適切に調整
プロタイプでは、既存のテーブルコーディネートは固定している。
